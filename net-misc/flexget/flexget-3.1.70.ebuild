...
DEPEND="dev-python/setuptools[${PYTHON_USEDEP}]
	...
	~dev-python/flask-restx-0.2.0[${PYTHON_USEDEP}]
	...
	~dev-python/plumbum-1.6.8[${PYTHON_USEDEP}]
...
src_prepare() {
	...
	sed -i -e "s/^certifi==2017.4.17/certifi==10001/" ${S}/requirements.txt || die
	...
	sed -i -e "s/^idna==2.8/idna==2.10/" ${S}/requirements.txt || die
	...
	sed -i -e "s/^more-itertools==7.2.0/more-itertools==8.5.0/" ${S}/requirements.txt || die
	#sed -i -e "s/^path.py==11.5.0/path.py==11.0.1/" ${S}/requirements.txt || die
	sed -i -e "s/^pillow==7.0.0/pillow==7.1.2/" ${S}/requirements.txt || die
	sed -i -e "s/^plumbum==1.6.3/plumbum==1.6.8/" ${S}/requirements.txt || die
